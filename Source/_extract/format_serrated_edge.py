# -*- coding: utf-8 -*-
"""Formatting pass: reflow Serrated Edge Source Texts into readable markdown.

Run AFTER extract_serrated_edge.py (expects raw PDF line breaks).
Does not format 00 - GM Adventure Brief.md or INDEX.md.
"""
...
# Page chrome / redundant scene banners (file H1 already has the title)
SKIP_EXACT = {
    "SHADOWRUN",
    "DENVER:",
    "SERRATED",
...
def strip_noise(s: str) -> str | None:
    ...
    if re.match(r"^\d+\s+AFTERMATH\s*$", s, re.I):
        return None
    if re.match(r"^AFTERMATH\s+\d+\s*$", s, re.I):
        return None
...
def reflow_body(body: str) -> str:
    ...
    while i < len(lines):
        ...
        if s.startswith("•") or s.startswith("- "):
            ...
            if bullet_buf is not None:
                flush_bullet()
            bullet_buf = [s.lstrip("•").strip() if s.startswith("•") else s[2:].strip()]
            i += 1
            continue
