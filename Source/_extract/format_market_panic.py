# -*- coding: utf-8 -*-
"""Formatting pass for Market Panic Source Texts chapters."""
...
BASE_H2: dict[str, str] = {
    ...
    "contents and credits": "Contents & Credits",
    "the first day of the rest of your life": "The First Day of the Rest of Your Life",
    "droning on": "Droning On",
    "dispatches from the edge": "Dispatches From the Edge",
    "waking up": "Waking Up",
    "the commute": "The Commute",
    "the work day begins": "The Work Day Begins",
    "the break room": "The Break Room",
    ...
    "table of organization": "Table of Organization",
    "war in the shadows": "War in the Shadows",
    "the monad matter": "The Monad Matter",
    "consensus 2.0": "Consensus 2.0",
    "draconic neo-feudalism": "Draconic Neo-Feudalism",
    "in the dragon's shadow": "In the Dragon's Shadow",
}
...
JP_HANDLE_KEYS = {
    ...
    "winterhawk", "frosty", "elijah", "kay st. irregular", "plan 9",
    "axis mundi", "piotr", "red", "stone", "thunderbird", "hard exit",
    "fianchetto", "balladeer", "aufheben", "kappa", "the smiling bandit",
    "marcos", "beaker", "lyran", "mana", "haze", "goat foot", "ecotope",
    "baka", "baka kekku", "takata", "mika", "chainmaker", "kosher",
}
        ...
        if s == ">":
            ...
            if i < len(lines):
                ...
                if re.match(r"^>\s*[A-Za-z0-9/]", t) and len(t) < 55:
                    ...
                    handle = "> " + t
                    i += 1
            # Flip to handle-then-body
            if handle:
                h = handle[2:].strip() if handle.startswith("> ") else handle.strip("> ").strip()
                out.append(f"> **{h}**")
            if body:
                out.append("> " + body)
            continue
...
def format_body(
    ...
    chapter_keys = {
        ...
        norm_key(chapter_title.replace("&", "and")),
    }
