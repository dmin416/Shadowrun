...
**RF identity-linked qualities (not shop SKUs):** Digital Doppelganger (needs SIN or Fake SIN ≥4); Day Job (needs SINner or Fake SIN ≥4); Erased (Fake SIN / licenses ≤3 months; Lifestyle caps); Bi-polar meds prescription needs SIN/Fake SIN
**RF PACKs with Fake SIN and/or credstick (4):** Intro Runner; Basic Runner; Advanced Runner; Bug-Out Bag
**SS (2 + services):** Fake DocWagon wrist band; Forger services; Uniforms; Fixer services (disguise)
**Sample gear only (no new shop tables):** RF/CT NPC gear lists using Core Fake SIN/license prices
**No local PDF shop SKU:** passport, visa, national ID card, legal license fee table, biometric-sample "extra fee" price; Complete Trog / Data Trails / Kill Code / Chrome Flesh add **no** Fake SIN buy rows
...
### Forgery skill (Core Skills; Using Forgery p.145)

- Skill: **Forgery (Logic)**. Specializations: Counterfeiting, Credstick Forgery, False ID, Image Doctoring, Paper Forgery.
- Create: Forgery + Logic Test. Limit = **Physical** (art, signatures, physical objects) or **Mental** (credsticks, documents, data-heavy). Apply Build/Repair modifiers; GM sets time (Extended interval OK). Hits = forgery **quality** = threshold to spot.
- Spot: Opposed Perception + Intuition [Mental] or Forgery + Intuition [Mental] vs quality. Hits ≥ quality -> exposed as fake.
- **Data forgeries** (credsticks, documents, SINs): can look identical, but **any use as the real thing fails** (transfer nuyen, pass SIN check, etc.). Shop **Fake SIN / Fake license** rows are the working Rating products; DIY Forgery is not a substitute buy table.

### Device Ratings (Core p.234; SIN checkers + credstick DR)

| Device type | DR | Examples |
| --- | --- | --- |
...
### Availability and licenses (Core ~419 + Street Gear)

- **R** Restricted: buy/own/transport only with special circumstances; solid citizen with legal SIN can get legal permits; runners use Fake SIN + Fake license. Fake license tied to Fake SIN; **if one exposed, the other becomes worthless**.
- **F** Forbidden: never OK for private citizens; no license makes it legal. Don't get caught.
...
| Fake SIN (Rating 1) | Core | FakeSIN | 1 | 3F | 2,500¥ | Random anybody; age/nationality/sex may not match; no supporting data. |
| Fake SIN (Rating 2) | Core | FakeSIN | 2 | 6F | 5,000¥ | Rough match; sex matches; age/nationality pretty close; no supporting data. |
| Fake SIN (Rating 3) | Core | FakeSIN | 3 | 9F | 7,500¥ | Good match; sex/age/nationality match; supporting data obviously fake. |
| Fake SIN (Rating 4) | Core | FakeSIN | 4 | 12F | 10,000¥ | Casually plausible; stats match; supporting data OK on cursory checks. Min for Day Job / Digital Doppelganger Fake SIN options (RF). |
| Fake SIN (Rating 5) | Core | FakeSIN | 5 | 15F | 12,500¥ | Good fit; all stats match; valid biometrics for another person with samples; some history. Extra sample fee unlisted. |
| Fake SIN (Rating 6) | Core | FakeSIN | 6 | 18F | 15,000¥ | Alternate life; all stats match; biometrics + samples; complete believable history. Extra sample fee unlisted. |
...
| Fake license (Rating 1) | Core | FakeLicense | 1 | 3F | 200¥ | Lowest craft quality vs checkers. |
| Fake license (Rating 2) | Core | FakeLicense | 2 | 6F | 400¥ | As Fake license; Rating 2 vs checkers. |
| Fake license (Rating 3) | Core | FakeLicense | 3 | 9F | 600¥ | Common on sample NPC gear (e.g. light pistol / SMG). |
| Fake license (Rating 4) | Core | FakeLicense | 4 | 12F | 800¥ | As Fake license; Rating 4 vs checkers. |
| Fake license (Rating 5) | Core | FakeLicense | 5 | 15F | 1,000¥ | Sample gear often lists specific types (driving, etc.). |
| Fake license (Rating 6) | Core | FakeLicense | 6 | 18F | 1,200¥ | Highest craft quality. |
...
## RF qualities that gate identity (not shop SKUs)

| Quality | Src | Karma | Identity rule |
| --- | --- | --- | --- |
| Digital Doppelganger | RF | 7 (positive) | Cross-linked digital identity: +2 threshold on Matrix Searches tracking this ID. Must attach to a specific SIN: real **SINner** or **Fake SIN Rating ≥ 4**. |
| Day Job | RF | 5-15 (negative) | Needs valid SIN: **SINner** or **Fake SIN Rating ≥ 4**. SINner: job pay already taxed. Fake burned / tied to crime: hard LE response; job lost until cleared. Burn job after warnings -> -2 Street Cred + lose 1 month salary. |
| Erased | RF | 8 (positive) | Prior SIN thoroughly wiped; Matrix searches empty; Public Awareness never above 1. Caps: Lifestyle never above Middle (High for 1 month requires following month Low or lower or systems slip). **Cannot use a Fake SIN or attached licenses for more than three months.** |
| Bi-polar (meds note) | RF | (quality) | Stabilizing meds 500¥/month need prescription + SIN (Fake SIN OK); street 100¥/daily dose. Identity gate only; full quality elsewhere. |

---

## SINner quality (Core p.84; chargen negative)

Not a shop SKU. **SINner (Layered)** bonus **5 to 25** Karma. Four tiers. Must always legally **broadcast** SIN. Legal purchases need a legal SIN (or working Fake SIN).

| Tier | Karma | Tax | Key rules |
| --- | --- | --- | --- |
| National SIN | 5 | 15% gross to nation | Citizen: vote, nation passport, national military/gov. Biometrics in national + GSINR (easy track). Nation sells personal data to corps -> ~3x spam, tailored. No megacorp affiliation. |
| Criminal SIN (Corp or National) | 10 | 15% gross to issuer | Replaces prior SIN. Must broadcast Criminal SIN (hiding = felony). Social hostility; denied many venues; hard legal employment. Can be held up to **48 hours** questioning when crime nearby; justice biased toward closing cases. Magic users with Criminal SIN: registered with LE; irregular checks for forbidden formulae/foci/gear. +1 Notoriety. Fail to update residence / evade oversight -> arrest. |
| Corporate Limited SIN | 15 | 20% gross to megacorp | Outside hire into megacorp; usually replaces National. In GSINR; often records Awakened. Can be wageslave / low security / enlisted military; not leadership, officer, or Special Forces. Extraction target even if inactive. SINless hostility (seen as sellout). |
| Corporate Born (Corporate SIN) | 25 | 10% gross to megacorp | Born/raised in megacorp hierarchy track (could have been officer/Special Forces/top secret). GSINR confirms valid SIN but **no extra detail** (corp-only records). If SINless learn of Corporate Born: suspicion to lethal hostility. Loyalty assumed by shadows. |

Full prose remains in Core Creating a Shadowrunner; this table is the mechanical extract for identity play.
...
- **Complete Trog / RF sample NPCs:** use Core Fake SIN/license prices; no alternate shop table.
- **Hard currency / ECC:** RF contact-payment flavor only; not certified-credstick SKUs.
- **Erased vs multi-SIN ops:** Erased caps Fake SIN use at 3 months; conflicts with long-term Fake SIN lifestyle if both taken (resolve per quality).

---

## Item index

Advanced Runner PACK; Basic Runner PACK; Bug-Out Bag; Certified credstick (Standard/Silver/Gold/Platinum/Ebony); Credit account (procedure); Day Job (RF quality gate); Digital Doppelganger (RF quality gate); Erased (RF quality gate); Fake DocWagon wrist band; Fake license (R1-6); Fake SIN (R1-6); Fixer services (SS disguise); Forger services; Forgery skill; Intro Runner PACK; Legal licenses (Item/Magic/Spell Weapon/Technomancy/Occupational); SIN verification; SINner quality; Uniforms (SS).
